...
class Order extends React.Component{
    ...
    renderGoods=()=>{
        ...
        const buyIimmediaGoods = JSON.parse(sessionStorage.getItem('buyIimmediaGoods'))
        let buyGoodsList = buyIimmediaGoods ? buyIimmediaGoods : selectGoods
        // console.log('selectGoods---', buyGoodsList)
        const divDom = [];
        for(let key in buyGoodsList){
    ...
    submitOrder=()=>{
        ...
        const cat_id = store.shoppingCarList.ids;
    ...
    render(){
        const {store} = this.props;
        const {buttonLoading,address_id,freight,invoiceList,switchAddModal,addressModal,defaultAddress,invoiceType,editAddressData,isEdit,invoiceModal,addInvoiceType} = this.state;
        const switchData = {
            switchAddModal,
                ...
                <div className="order-information-box">
                    ...
                    <div className="order-other-information-box">
                        ...
                        <div className="order-amount-detail">
                            <div>
                                <p>
                                    <span>{store.shoppingCarList.selectNumber}</span>件商品，商品总价：
                                </p>
                                <span>￥{store.shoppingCarList.selectPrice}</span>
                            </div>
                            <div>
                                <p>
                                    运费：
                        ...
                        <div className="order-amount-number">
                            <h2>
                                应付总额：
                                <span>￥{(Number(store.shoppingCarList.selectPrice) + Number(freight)).toFixed(2)}</span>
                            </h2>
                            {
                                defaultAddress.length == 0 && <p>暂无地址信息，立即新增一个吧~</p>
                            }
