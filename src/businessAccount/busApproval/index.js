...
import {Table,Input,Select,Button,message,Switch,Modal,Pagination,Popover} from "antd"
...
export default class busApproval extends React.Component{
    ...
    render(){
        const {columns,data,page_size,total,page_number} = this.state;
        // 分页配置
        const pagination={
            total:total,
            pageSize:page_size,
    ...
    // 新增审批流
    addApproval = e =>{
        history.push('/BusinessAccount/add')
    }
    // 编辑审批流
    editApproval = record =>{
        //console.log(record);
        ...
        api.axiosPost('getApprovalById',data).then((res)=>{
            if(res.data.code === 1){
                ...
                history.push('/BusinessAccount/edit')
            }else{
